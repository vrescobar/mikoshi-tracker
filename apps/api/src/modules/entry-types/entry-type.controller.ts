...
import { getEntryTypeBySlug, listActiveEntryTypes } from "./entry-type.repository";
...
export async function listEntryTypesHandler(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    const items = listActiveEntryTypes(request.server.sqlite);
...
export async function getEntryTypeHandler(request: FastifyRequest, reply: FastifyReply) {
  try {
    ...
    const entryType = getEntryTypeBySlug(request.server.sqlite, slug);
