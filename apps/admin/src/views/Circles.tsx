...
import { api, type AdminCircle } from "../lib/api";
import { useAsync, errorMessage } from "../lib/useAsync";
import { navigate } from "../lib/router";
import { DataTable, Pager, SearchBox, type Column } from "../components/DataTable";
import { Field, Modal, Pill, useToast } from "../components/ui";
  ...
  return (
    <>
      ...
      <DataTable
        ...
        error={list.error}
        onRowClick={(c) => navigate(`circles/${encodeURIComponent(c.id)}`)}
      ...
      {creating && (
        <CreateCircleModal
          ...
          onDone={(id) => {
            setCreating(false);
            if (id) navigate(`circles/${encodeURIComponent(id)}`);
            else list.reload();
          }}
        />
      )}
