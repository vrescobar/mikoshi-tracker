...
import { api, type AdminCircle, type CircleContestConfig, type CircleMember } from "../lib/api";
    ...
    { header: "Joined", cell: (m) => <span className="dim">{fmtDate(m.joinedAt)}</span> },
    {
      ...
      cell: (m) => (
        <div className="row-actions">
          <button className="btn ghost sm" onClick={() => navigate(`users/${encodeURIComponent(m.userId)}`)}>
            Profile
          </button>
          {m.role !== "owner" && (
