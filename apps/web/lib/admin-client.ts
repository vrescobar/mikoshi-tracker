...
/**
 * Admin (god-mode) API client, ported from the retired apps/admin SPA. It
 * authenticates with the better-auth session cookie of a signed-in admin —
 * the API accepts admin sessions as operators alongside the bearer keys that
 * bots keep using — so there is no pasted key and no LoginGate.
 */

// The v1 envelope, shared with the API/MCP (cross-frontend SSOT).
...
async function call<T>(method: Method, path: string, opts: CallOptions = {}): Promise<T> {
  ...
  try {
    response = await apiFetch(`${base}${path}`, {
      method,
      headers,
      body: method === "GET" ? undefined : JSON.stringify(opts.body ?? {}),
    });
  } catch (cause) {
    throw new AdminApiError("NETWORK", cause instanceof Error ? cause.message : "Network error");
  }
  ...
  if (raw && typeof raw === "object" && "ok" in raw) {
    ...
    if (!payload.ok) {
      throw new AdminApiError(payload.code, payload.error);
    }
    return payload.data;
  }
  ...
  if (!response.ok) {
    ...
    throw new AdminApiError(err?.code ?? "INTERNAL_ERROR", err?.message ?? `Request failed (${response.status})`);
  }
  return raw as T;
}
...
// ── Admin endpoints (god-mode reads + admin mutations) ────────────────────────
...
export const adminApi = {
  ...
  // ── Legacy /api/admin lifecycle + identity ──
...
export function asUser(userId: string) {
  ...
  return {
    ...
    updateMember: (
      circleId: string,
      membershipId: string,
      patch: { role?: "owner" | "member"; externalId?: string | null },
    ) =>
