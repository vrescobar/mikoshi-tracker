...
cites the plan section it implements. Do not duplicate check-in mutation logic;
reuse `apps/api/src/modules/checkins/checkin.service.ts`. Do not regress the
single-user flow or `@haaabit/mcp`.
...
- [ ] **02** Circle contracts — create `packages/contracts/src/circles.ts`
      ...
      (both circle-token and session-authenticated surfaces), exported
      consistently with `habits.ts`/`today.ts` and wired into the package
      index. Authored before the API module so handlers and the web client
      import one definition. See `PLAN-CIRCLES.md` §5, §8 (H2).
...
- [ ] **05** Circles module scaffold — create
      ...
      `apps/api/src/server.ts`. See `PLAN-CIRCLES.md` §5.
...
- [ ] **09** Circle write endpoints — implement
      ...
      `userId` and `source: "circle"`. `undo` reverts **only** the latest
      mutation of the day whose `source` is `"circle"`; if the latest is
      `web`/`ai` it returns `409 UNDO_NOT_CIRCLE_SOURCED` and mutates nothing.
      See `PLAN-CIRCLES.md` §5.1, §5.2bis, §5.3.
...
- [ ] **14** Circle-token denial matrix — Vitest in `apps/api` covering
      ...
      leaking un-shared habits, and the `undo` scope rule (§9.9): circle
...
- [ ] **15** Management authorization tests — Vitest covering `PLAN-CIRCLES.md`
...
> does and its consequences before the user acts (`PLAN-CIRCLES.md` §7.1).
...
- [ ] **22** Explanatory copy — for every Circles action that shares data or
      ...
      what it does and its consequences, per `PLAN-CIRCLES.md` §7.1. Copy is
      added as i18n strings (English + Chinese) so task 24 can translate it.
...
- [ ] **24** Spanish translation of the whole GUI — add a Spanish (`es`)
      ...
      See `PLAN-CIRCLES.md` §7.2.

## Phase 10 — Final verification

- [ ] **25** Full acceptance pass — confirm `PLAN-CIRCLES.md` §10: `pnpm -r
      build` and workspace typecheck green after Prisma regen, `pnpm -r lint`
      green, `pnpm --filter @haaabit/api test` green (incl. the denial matrix
      and undo-scope test), `pnpm test:e2e` and `pnpm verify:openclaw` green
      with no single-user / `@haaabit/mcp` regressions, the Circles web
      section complete and explanatory, and the whole GUI translated to
